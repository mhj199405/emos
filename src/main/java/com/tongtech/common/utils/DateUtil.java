...
public class DateUtil {
     ...
     * @return 之后或之后的日期
     */
    public static String getAfterOrPreOnNowTime(String node, Long num) {
        LocalDateTime now = LocalDateTime.now();
        return getAfterOrPreOnBaseDateTime(now, node, num);
    }


    /**
     * 基于指定的时间（在此时间的基础上减少或者增加一段时间）日期之后（之后）的节点事件
     * <ul>
     * 比如当前时间为：2019-03-30 10:20:30
     * </ul>
     * node="hour",num=5L:2019-03-30 15:20:30
     ...
     * node="year",num=1L:2020-03-30 10:20:30
     *
     * @param baseDateTime 基准时间（在此时间的基础上减少或者增加一段时间）；
     * @param node         节点元素（“year”,"month","week","day","huor","minute","second"）
     * @param num          第几天（+：之后，-：之前）
     * @return 之后或之后的日期
     */
    public static String getAfterOrPreOnBaseDateTime(LocalDateTime baseDateTime, String node, Long num) {
        if (HOUR.equals(node)) {
            return baseDateTime.plusHours(num).format(yyyyMMddHHmmss_EN);
        }
        else if (DAY.equals(node)) {
            return baseDateTime.plusDays(num).format(yyyyMMddHHmmss_EN);
        }
        else if (WEEK.equals(node)) {
            return baseDateTime.plusWeeks(num).format(yyyyMMddHHmmss_EN);
        }
        else if (MONTH.equals(node)) {
            return baseDateTime.plusMonths(num).format(yyyyMMddHHmmss_EN);
        }
        else if (YEAR.equals(node)) {
            return baseDateTime.plusYears(num).format(yyyyMMddHHmmss_EN);
        }
        else if (MINUTE.equals(node)) {
            return baseDateTime.plusMinutes(num).format(yyyyMMddHHmmss_EN);
        }
        else if (SECOND.equals(node)) {
            return baseDateTime.plusSeconds(num).format(yyyyMMddHHmmss_EN);
        }
        else {
     ...
     * @example startDate="2019-02-28",endDate="2019-03-05",period="day"
